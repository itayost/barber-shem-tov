// src/components/common/SimpleCTA.tsx - Updated
'use client';
...
const SimpleCTA: React.FC<SimpleCTAProps> = ({
  ...
  return (
    <div className={`w-full ${className}`}>
      {/* Main Apply Button - Changed from WhatsApp */}
      <motion.button
        onClick={handleApply}
        className={`
      ...
      >
        ...
        <button
          ...
          className="text-green-500 hover:text-green-400 transition-colors flex items-center gap-1 text-sm"
        >
          <svg 
            className="w-4 h-4"
            fill="currentColor" 
        ...
        {showPhone && (
          <button
            onClick={handlePhone}
            className="text-gold hover:text-gold/80 transition-colors flex items-center gap-1 text-sm"
          >

// File: src/components/academy/AcademyTestimonials.tsx
'use client';
...
const AcademyTestimonials = () => {
  ...
  const goToNextSlide = useCallback(() => {
    const newIndex = (currentIndex + 1) % academyTestimonials.length;
    goToSlide(newIndex);
  }, [currentIndex, goToSlide]);

  const goToPrevSlide = useCallback(() => {
    const newIndex = (currentIndex - 1 + academyTestimonials.length) % academyTestimonials.length;
    goToSlide(newIndex);
  }, [currentIndex, goToSlide]);
  ...
  return (
    <section className="py-section-mobile md:py-section bg-brown bg-opacity-10 relative overflow-hidden" dir="rtl">
      ...
      <div className="container mx-auto px-6 text-center relative z-10">
        <h2 className="font-heebo text-h3 md:text-h2 mb-6">סיפורי הצלחה</h2>
        <p className="text-lightgrey max-w-xl mx-auto mb-16">
          מה הבוגרים שלנו אומרים על הלימודים באקדמיה והדרך להצלחה
        </p>
              ...
              >
                {academyTestimonials.map((testimonial) => (
                  ...
                  >
                    ...
                    {/* Student name with graduation year */}
                    <div className="mb-1">
                      <p className="font-medium text-gold mb-2">{testimonial.name}</p>
                      {testimonial.year && (
                        <span className="text-lightgrey text-sm">בוגר {testimonial.year}</span>
                      )}
                    </div>
                    
                    {/* Course & instructor */}
                    <p className="text-lightgrey mb-4 text-sm">
                      קורס: {testimonial.course} | מדריך: {testimonial.instructor}
                    </p>
